...
class CorpusDataLoad:
    ...
    class Dataset:
                ...
                for data_line in datum:
                    for part_line in data_line['body']:
                        for text_line in part_line:
                            yield self._tokenizer.encode(text_line['text'], return_tensors="tf")[0]
        ...
        def dataset_text(self) -> tf.data.Dataset:
            return tf.data.Dataset.from_generator(self.data_text_line_gen, output_signature=tf.TensorSpec(shape=(None),dtype=tf.int32))
        
        @property
        def dataset_part(self) -> tf.data.Dataset:
            return tf.data.Dataset.from_generator(self.data_part_gen, output_signature=tf.TensorSpec(shape=(None), dtype=tf.int32))
        
        @property
        def dataset_body(self) -> tf.data.Dataset:
            return tf.data.Dataset.from_generator(self.data_body_gen, output_signature=tf.TensorSpec(shape=(None),dtype=tf.int32))
...
if __name__ == "__main__":
    ...
    tf_dataset = tf_dataset.shuffle(128).padded_batch(8, padded_shapes=([None]))
    i = 0
