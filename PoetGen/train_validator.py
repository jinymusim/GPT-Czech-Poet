        ...
        if req_val == 'metre_ids':
            for i in range(datum['input_ids'].shape[0]):
                res = model.validate(input_ids=datum["input_ids"][i,:].reshape(1,-1).to(device),
                                    ...
                                    year=None)['acc']
                true_hits += res
                per_value_accs[data[i][req_val][i]] = per_value_accs.get(data[i][req_val][i], []) + [res]
                count +=1
        else:      
...
def main(args):
    ...
    with open(args.result_file, 'a') as file:
        print(f"### NEW NEW FORMAT! ### {type(tokenizer.backend_tokenizer.model).__name__} ### {time_stamp} ### Syllable: {str(args.syllables)} ### SAM Training: {str(args.SAM)}", file=file)
