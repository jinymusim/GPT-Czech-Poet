...
def main(args):
    ...
    with open(args.result_file, 'a') as file:
        print("\n",file=file)
        print(f"### LESS RHYME NEW NEW NEW FORMAT! ### {type(tokenizer.backend_tokenizer.model).__name__} ### {time_stamp} ### Syllable: {str(args.syllables)} ### SAM Training: {str(args.SAM)}", file=file)
