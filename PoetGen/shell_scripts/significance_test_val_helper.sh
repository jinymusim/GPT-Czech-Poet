...
python3 test_model_significance.py --base_val_model_path_full=$BASE --improved_val_model_path_full=$IMPROVED --base_validator_tokenizer_model=$BASETOK --improved_validator_tokenizer_model=$IMPROVEDTOK --base_val_syllables=$BASESYL --improved_val_syllables=$IMPROVEDSYL --validator_type=$TYPE --base_meter_context=$BASECONTEXT --improved_meter_context=$IMPROVEDCONTEXT