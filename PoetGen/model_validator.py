...
class ModelValidator:
    ...
    def decode_helper(self, type:str):
        ...
        """
        if type  == "basic":
            tokenized_poet_start = self.tokenizer.encode(random.choice(NORMAL_SCHEMES), return_tensors='pt', truncation=True)
        ...
        if type == "forced":
            rhyme = random.choice(NORMAL_SCHEMES)
...
parser.add_argument("--default_tokenizer_model", default=os.path.abspath(os.path.join(os.path.dirname(__file__), "utils", "tokenizers", "BPE", "processed_tokenizer.json")), type=str, help="Default Model from HF to use")
...
parser.add_argument("--model_path_full", default=os.path.abspath(os.path.join(os.path.dirname(__file__),'backup_LMS', "Processed-BPE-gpt-cz-poetry-base-e16e16")),  type=str, help="Path to Model")
